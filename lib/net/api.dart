...
class Api {
  ...
  static Future<LoginResponse> register(
    ...
    map['phone'] = account;
    map['password'] = pwd;
    map['verificationCode'] = pwdRepeat;
  ...
  ///点赞接口
  static Future<CommonResponse> likeVideo(String id, bool isLike) async {
    var result = await HttpManager.getInstance().put(
        url: '/api/like/feed/' + id + "/" + isLike.toString(),
        cancelTokenTag: '');
  ...
  static Future<UploadTokenResponse> getSingleUploadToken(
    ...
    mapTemp['ext'] = filePathList[0];
    mapTemp['key'] = filePathList[1];
    mapTemp['size'] = filePathList[2];
    mapTemp['name'] = filePathList[3];
  ...
  static Future<bool> uploadSingleFile(
    ...
    UploadTokenToken tokenToken = tokenResponse.tokens;
    ...
    HttpManager.getInstance().finishFileUpload(tokenResponse.tokens.fileId);
    return success;
  }
  ...
  static Future<UserWorkListResponse> getUserFeedList(
      String uid, int cursor, int count) async {
    var result = await HttpManager.getInstance().get(
        url: HttpConstant.userFeedList + '/$uid/$cursor/$count',
  ...
  static Future<FeedListResponse> getHotFeedList(int cursor, int count) async {
    var result = await HttpManager.getInstance().get(
        url: HttpConstant.hotFeedList + '/$cursor/$count',
  ...
  ///评论接口
  static Future<CommonResponse> sendCommentByFeedId(
      String str, String feedId) async {
    print(str);
    print(feedId);
    Map<String, String> mapTemp = HashMap();
    mapTemp['details'] = str;
    mapTemp['id'] = feedId;
    var result = await HttpManager.getInstance()
        .post(url: '/api/comment/add', data: mapTemp, cancelTokenTag: 'follow');
    print(result);
    return result;
  }

  ///获取当前资源的评论接口
  static Future<CommentListResponse> getCommentListByFeedId(id) async {
    var result = await HttpManager.getInstance().get(
        url: '/api/comment/getCommentList/$id',
        cancelTokenTag: 'getCommentList');
    return CommentListResponse().fromJson(result);
  }

  ///获取当前是否关注
  static Future<bool> isFollow(uid) async {
    var result = HttpManager.getInstance()
        .get(url: '/api/user/isMyFollow/$uid', cancelTokenTag: 'isFollow');
    print('cao$result');
    return result;
  }
