...
class _UserInfoEditPageState extends State<UserInfoEditPage> {
        ...
        child: Column(
          children: [
            ...
            Obx(
              () => ItemTurnWidget(
                title: '抖音号',
                rightTitle: userController.userInfoResponse.value.id.toString(),
                onClick: () {},
            ...
            Obx(() {
              String gender = userController.userInfoResponse.value.gender;
  ...
  void _uploadHeaderImg(int typeImgSource) async {
    ...
    bool success =
        await uploadController.uploadSingleFile(imgSuffix, imgCamera);
    if (success) {
      userController.userInfoResponse.value.portrait =
          uploadController.uploadResponse.tokens.photoResultUrl;
  ...
  ///展示性别选择框
  void _showPickerGender(String genderInt) {
    showModalBottomSheet(
        context: context,
        shape: RoundedRectangleBorder(
          borderRadius: const BorderRadius.only(
        ...
        builder: (context) => ClickItemListWidget(
              ...
              onClick: (int position) {
                switch (position) {
                  case 0:
                    userController.userInfoResponse.value.gender = "MALE";
                    break;
                  case 1:
                    userController.userInfoResponse.value.gender = "FEMALE";
                    break;
                }
                userController.updateUserInfo();
                Get.back();
